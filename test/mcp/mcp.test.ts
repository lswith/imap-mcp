...
import type { WriteService } from "../../src/writes";
    ...
    new StreamableHTTPClientTransport(ENDPOINT, {
      ...
      fetch: (input, init) =>
        handleRequest(
          ...
          env,
          authenticated(),
          // Injected, always: the default writer opens a real IMAP connection,
          // and a write test that means to reach the mailbox layer has to say
          // so by driving test/sync instead.
          writer,
        ),
    }),
  );

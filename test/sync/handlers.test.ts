...
describe("the write entry point", () => {
  ...
  it("is reachable through the write service, and offers nothing else", async () => {
    const error = vi.spyOn(console, "error").mockImplementation(() => {});
    const service = createWriteService(syncEnv({ IMAP_USER: undefined }));

    // The env is deliberately unconfigured: this asserts the service is wired
    // to the handlers, which every test above covers properly. What it also
    // asserts is the surface — three methods, and nothing else for the tool
    // layer to reach.
    await expect(service.flagMessage(flagRequest())).resolves.toMatchObject({ ok: false });
    await expect(service.moveMessage(moveRequest())).resolves.toMatchObject({ ok: false });
    await expect(service.createDraft(draftRequest())).resolves.toMatchObject({ ok: false });

    error.mockRestore();
    expect(Object.keys(service).sort()).toEqual(["createDraft", "flagMessage", "moveMessage"]);
  });
});

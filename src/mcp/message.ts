 ...
 * (src/sync/normalise.ts): the HTML is already reduced, hidden
 ...
 *
 * Nothing here opens an IMAP connection: the protocol client is importable
 * only from src/imap/cf-imap-mailbox.ts (enforced by lint), and reads come
 * from D1 alone.
 */
 ...
 *
 * The stored ceiling is 256 KB (`MAX_BODY_CHARS` in src/sync/normalise.ts), which is

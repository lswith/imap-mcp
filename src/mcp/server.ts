 ...
 *
 * The writes reach the mailbox through the write service (src/sync/handlers.ts),
 * which is where every refusal is decided — and there is deliberately no tool
 * here that can send mail or delete it.
 */
 ...
 * bindings arrive by closure from the fetch handler — see src/mcp/handler.ts.
 * The write service and the Access context arrive the same way; the context is
 * used only by the write tools, because `getIdentity()` is a call rather than a
 * property, and a search does not pay for an identity lookup it has nothing to
 * do with.
 */

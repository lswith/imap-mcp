 ...
 * longer means open: every request is refused unless Cloudflare Access
 * authenticated it for this application. See src/access.ts and
 * ../../docs/access.md.
 */
...
/**
 * The worker, as a plain function of its three inputs.
 *
 * Exported alongside the default handler the way packages/sync exports
 * `handleScheduled` and `handleQueue`, and for the same reason: a test cannot
 * reach this path through the normal entry point. `SELF` from `cloudflare:test`
 * is a service binding, and Cloudflare documents that Access deliberately does
 * not propagate `ctx.access` across one — so a request made through
 * `SELF.fetch` can never carry an authenticated context, however the harness is
 * configured. Calling this directly is the only way to exercise the

 ...
 * rather than a caching one: fetching on demand would put IMAP on the read
 * path, so the app-specific password would have to exist in two workers. The
 * bytes could not live in D1 in any case — its per-value limit is 2 MB.
 *
...
import type { MailboxAttachment, MailboxMessage } from "@imap-mcp/imap";
